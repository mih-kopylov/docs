...
Ory is a software infrastructure provider building a global zero trust network for humans, robots, devices, and logical services.
Ory fuses open source software (Ory, Kubernetes, Crossplane, Cockroach, Linux, Istio) and open standards (Ory, OAuth2, OpenID
Connect, MITREid, WebAuthn, TOTP) with centuries in combined open source, security, operational, and industry expertise.

[Ory Cloud](https://console.ory.sh)'s core infrastructure components are open-source to foster collaboration, reduce security
supply-chain risk, make security accessible, and introduce the open standard for internet security. Being open source, Ory
...
Ory is production ready, on track to secure one trillion API requests in 2022, and improved daily.
